...
export default async function handler(req, res) {
  ...
  try {
    // GET - List all files
    if (req.method === 'GET') {
      const { blobs } = await list({ prefix: 'csv-tables/' });

      const files = {};
      for (const blob of blobs) {
        const filename = blob.pathname.replace('csv-tables/', '');
        try {
    ...
    if (req.method === 'POST') {
      ...
      const blob = await put(`csv-tables/${filename}`, content, {
        access: 'public',
    ...
    if (req.method === 'DELETE') {
      ...
      const { blobs } = await list({ prefix: `csv-tables/${filename}` });
